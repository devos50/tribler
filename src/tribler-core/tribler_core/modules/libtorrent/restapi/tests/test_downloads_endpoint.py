...
@pytest.mark.asyncio
async def test_get_peers_illegal_fields_ascii(enable_chant, enable_api, test_download, mock_dlmgr, mock_lt_status, session):
    """
    ...
    assert 1 == len(response_dict["downloads"])
    assert "peers" in response_dict["downloads"][0]
    assert 1 == len(response_dict["downloads"][0]["peers"])
    assert 'have' not in response_dict["downloads"][0]["peers"][0]
    assert 'uTorrent 1.6.1' == response_dict["downloads"][0]["peers"][0]['extended_version']


@pytest.mark.asyncio
async def test_get_peers_illegal_fields_unknown(enable_chant, enable_api, test_download, mock_dlmgr, mock_lt_status, session):
    """
    Testing whether illegal fields are stripped from the Libtorrent download info response.
    """
    session.dlmgr.get_downloads = lambda: [test_download]
