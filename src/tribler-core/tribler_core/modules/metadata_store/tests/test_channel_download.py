...
async def test_channel_update_and_download(enable_chant, enable_libtorrent, channel_tdef, channel_seeder_session, session):
    """
    ...
    with db_session:
        ...
        assert 1565621688018 == channel2.timestamp
        assert 8 == session.mds.ChannelNode.select().count()
