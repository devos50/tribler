...
@pytest.mark.asyncio
async def test_regen_personal_channel_damaged_torrent(enable_chant, personal_channel, channel_manager, mock_dlmgr, session):
    """
...
async def test_regenerate_channel_torrent(enable_chant, personal_channel, channel_manager, mock_dlmgr, session):
    ...
    assert 1 == len(downloads_to_remove)
...
@pytest.mark.asyncio
async def test_check_and_regen_personal_channel_torrent(enable_chant, personal_channel, channel_manager, mock_dlmgr, session):
    with db_session:
        chan_pk, chan_id = personal_channel.public_key, personal_channel.id_
        chan_download = Mock()
...
async def test_check_channels_updates(enable_chant, personal_channel, channel_manager, mock_dlmgr, session):
    ...
    with db_session:
        ...
        session.dlmgr.get_download = lambda _: MockDownload()
