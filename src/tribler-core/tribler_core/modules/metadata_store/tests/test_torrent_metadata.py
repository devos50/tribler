...
def test_get_magnet(metadata_store):
    ...
    torrent_metadata2 = metadata_store.TorrentMetadata.from_dict({'title': u'\U0001f4a9', "infohash": random_infohash()})
...
def test_wildcard_search(metadata_store):
    ...
    assert 0 == len(metadata_store.TorrentMetadata.search_keyword("*")[:])
    assert 1 == len(metadata_store.TorrentMetadata.search_keyword("foobl*")[:])
    assert 2 == len(metadata_store.TorrentMetadata.search_keyword("foo*")[:])
    assert 1 == len(metadata_store.TorrentMetadata.search_keyword("(\"12\"* AND \"foobl\"*)")[:])
