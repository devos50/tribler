...
def id_to_binary_string(node_id):
    return format(int(hexlify(cast_to_bin(node_id)), 16), '0160b')


def distance(a, b):
    return int(hexlify(cast_to_bin(a)), 16) ^ int(hexlify(cast_to_bin(b)), 16)
...
class DHTHealthManager(TaskManager):
    ...
    def finalize_lookup(self, infohash):
        ...
        self.lookup_deferreds[infohash].callback({"DHT": [{"infohash": hexlify(infohash), "seeders": seeders, "leechers": peers}]})
    ...
    def received_bloomfilters(self, node_id, bf_seeds=bytearray(256), bf_peers=bytearray(256)):
        ...
        for infohash in self.lookup_deferreds.keys():
        ...
        self.bf_seeders[closest_infohash] = DHTHealthManager.combine_bloomfilters(self.bf_seeders[closest_infohash], bf_seeds)
        self.bf_peers[closest_infohash] = DHTHealthManager.combine_bloomfilters(self.bf_peers[closest_infohash], bf_peers)
